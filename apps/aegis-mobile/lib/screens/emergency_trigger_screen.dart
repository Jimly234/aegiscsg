...
class _EmergencyTriggerScreenState extends State<EmergencyTriggerScreen> with SingleTickerProviderStateMixin {
  ...
  void _onPanStart(DragStartDetails details) {
    ...
    setState(() { _isHolding = true; _holdProgress = 0.0; _statusMessage = ''; });
    _startHoldTimer();
  }

  void _onPanEnd(DragEndDetails details) {
    if (_isSending) return;
    setState(() { _isHolding = false; _holdProgress = 0.0; _statusMessage = 'Release to cancel'; });
  }
  ...
  void _startHoldTimer() async {
    ...
    for (int i = 0; i < steps; i++) {
      ...
      if (mounted && _isHolding) setState(() { _holdProgress = (i + 1) / steps; });
    }
    if (_isHolding && mounted && !_isSending) _triggerEmergency();
  }
  ...
  Future<void> _triggerEmergency() async {
    ...
    try {
      ...
      try { networkType = (await _connectivity.checkConnectivity()).toString(); } catch (_) {}
      ...
      if (mounted) {
        Navigator.of(context).pushReplacementNamed('/post_alert', arguments: {'apiSuccess': alertResult != null, 'smsCount': smsSent, 'latitude': position.latitude, 'longitude': position.longitude});
      }
          ...
          return Container(width: double.infinity, height: double.infinity,
            ...
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              if (!_alertSent) ...[
                ...
                Text(_isSending ? _statusMessage : _statusMessage.isNotEmpty ? _statusMessage : 'for 3 seconds to activate emergency', style: TextStyle(color: _isSending ? Colors.orange : Colors.white54, fontSize: 14)),
